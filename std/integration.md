# Интеграция и обмен данными

Стандарты по интеграции: обмен данными, EnterpriseData, таймауты, внешние ресурсы.

| № | Наименование | Ссылка |
|---|--------------|--------|
| 637 | Настройка обмена данными для классификаторов между различными информационными базами | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/637.md) |
| 701 | Разработка планов обмена с отборами | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/701.md) |
| 748 | Таймауты при работе с внешними ресурсами | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/748.md) |
| 771 | Интеграция прикладных решений через формат EnterpriseData | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/771.md) |
| 794 | Ограничения на использование внешних ресурсов | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/794.md) |
| 798 | Использование ботов системы взаимодействия | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/798.md) |
| 799 | Разработка правил регистраций | [ссылка](https://github.com/zeegin/v8std/blob/main/docs/std/799.md) |