...
if [[ ${#X5_FILES[@]} -gt 0 ]]; then
    ...
    # 6-й полевой тест (TASK-001, прогон конвейера): &&Маркер — экранированный маркер
    # подстановки СтрЗаменить (текст запроса «|&&БлокОтбора»), а не параметр.
    # Вычищаем &&Имя до извлечения токенов; одиночный &Имя остаётся параметром.
    qparams=$(sed 's/&&[А-Яа-яA-Za-zЁё_]*//g' "${X5_FILES[@]}" 2>/dev/null \
